...
(defn ^:export make-shape-map
  ...
  (let [init_eqs_str (goog.object/get s "init_eqs_str")
        ...
        per-frame-init-eqs (when (seq per-frame-init-parse) (eqp/emit version per-frame-init-parse))
        ...
        per-frame-eqs (when (seq per-frame-parse) (eqp/emit version per-frame-parse))
...
(defn ^:export make-wave-map
  ...
  (let [init_eqs_str (goog.object/get s "init_eqs_str")
        ...
        per-point-eqs (when (seq per-point-parse) (eqp/emit version per-point-parse))
...
(defn ^:export convert-basic-preset
  ...
  (let [per-frame-init-parse (when init-eqs-str (eqp/parse init-eqs-str))
        ...
        per-pixel-eqs (when (seq per-pixel-parse) (eqp/emit version per-pixel-parse))
